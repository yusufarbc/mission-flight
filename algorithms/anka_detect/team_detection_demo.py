...
class circle_cv:
    ...
    def preprocess(self):
        hsv_img = cv2.cuda.cvtColor(self.image, cv2.COLOR_BGR2HSV)
        
        return hsv_img
    
    def detect_circle(self, hsv_img):
        mask = cv2.cuda.inRange(hsv_img, self.min_red, self.max_red)
...
if __name__ == "__main__":
    ...
    while(cap.isOpened()):
        ...
        detect_circle = circle_cv(min_red, max_red, img_gpu)

        hsv_img = detect_circle.preprocess()
        mask = detect_circle.detect_circle(hsv_img).download()
