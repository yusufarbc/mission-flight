...
Mission and flight software for the IEEE BTU ANKA fixed-wing UAV team (Teknofest International UAV Competition 2022). This repository contains the mission controller, perception helpers, detection experiments, assets, and design documentation we used during the competition.


## Quick links
- Landing page: `index.html`
- Competition report (PDF): `BTU_ANKA_Report_2022.pdf`
- Preliminary Design Review (markdown source): `PDR_Teknofest_2022_PDR.md`

## Repository layout
- `intuitive_shot_flight_algorithm/` - Mission controller, perception helpers, waypoints, and runtime log.
...
Notes:
- Adjust `start_wp`, `stop_wp`, and throw timing values in `flight_controller.py` to match your Mission Planner file before flight.
- `perception.py` performs red-target detection; tune HSV bounds if your marker color differs.
...
- For the full team list and contacts, add an appendix with names and responsibilities.

## Mission problem statement
In a conflict scenario, fires can ignite along the front line, threatening soldiers, vehicles, and civilians. Direct firefighting may be impossible under enemy fire. The UAV mission aims to mitigate threat and enable safe firefighting action.
...
- Flight area defined by competition map and ground control station.
...
- Perform manual takeoff; after ~5 seconds of stable climb, switch to AUTO to start the autonomous mission.

## Autonomous flight sequence
1) Follow the pre-uploaded route from the ground control station (Mission Planner recommended).  
2) Release smoke payload at the first target zone (blue).  
3) Release water payload at the second target zone (red).  
4) After both payloads are released, mission completes; return and land manually.
...
- Add table with mechanical parts, materials, dimensions, quantities, and mass.
...
- Planned distance: ~400 m per lap; two laps -> ~800 m total.
...
1) Add mass table with measured/estimated values and compute take-off weight plus CG envelope.  
...
- Provide component list and masses used in the design.
